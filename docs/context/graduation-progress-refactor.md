...
- 리팩토링 이유: 학과 개편으로 동일 `establishedDepartmentName` 아래 여러 학과 ID가 존재하며, `student.getMajor()`가 있든 없든 단일 ID만으로 졸업 요건을 찾으면 사용자가 실제로 속한 학과의 요구 사항을 놓치는 문제가 발생한다. Major가 null일 때 Department를 쓰는 기존 분기는 단지 Major가 존재하지 않는 학과가 있기 때문이며, 근본 원인은 단일 학과 ID 조회 전략이다. 복수전공(`student.getSecondaryMajor() != null`) 조합 역시 동일 학과명으로 다수 ID가 나뉘어 있어 1차 시도 실패 시 다른 후보 학과를 재탐색해야 한다.
...
## 기존 문제점 (As-Is)
- 구조적 문제: `resolvePrimaryMajorId`가 Major 존재 여부에 따라 단일 학과 ID만 반환해 학과 개편(동일 계열명, 상이한 ID) 시 요구사항 데이터를 한 번만 탐색하고 실패하면 회복하지 못한다. Major가 존재하는 학생도 동일 문제를 겪는다. 복수전공 학과(`secondaryMajor`) 역시 단일 ID만으로 복수전공 졸업 요건을 조회해 실패 시 예외로 끝나며, 후보 학과 탐색 수단이 없다.
...
- 구조 변화 요약: `GraduationService`는 Major 존재 여부와 관계없이 `Department.establishedDepartmentName` 기반으로 동일 명칭 학과 ID 리스트를 가져와 순차적으로 졸업 요건 데이터를 조회하고, 최초로 성공하는 ID를 `primaryDepartmentId`로 사용한다. Major 정보가 있으면 해당 Department를 우선 후보에 포함한다. 복수전공이 존재하면 동일 로직으로 secondary 후보 리스트를 만들고 `getDualMajorRequirementsWithCache`로 검증해 성공하는 조합을 선택한다.
...
- 추가해야 할 테스트: `GraduationServiceTests`(또는 동등 단위)에서
  ...
  4) 복수전공이 있는 학생은 secondary 학과 후보를 순차적으로 탐색해 성공한 조합으로 `getDualMajorAreaProgress`를 호출함
  을 검증한다.
