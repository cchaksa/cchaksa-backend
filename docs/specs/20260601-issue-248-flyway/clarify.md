...
| 2026-06-01 | dev/prod Lambda runtime profiles disable Spring Boot Flyway auto-run. | Avoids cold-start migration work, schema history lock contention, API Lambda DDL privileges, and migration coupling to request-serving runtime. |
| 2026-06-01 | Keep `refresh_token.expiry` as `TIMESTAMP(6) NOT NULL` in `V1`. | dev/prod both report `timestamp without time zone`, precision `6`, `NOT NULL`; the Java field is `java.util.Date`, and DB session timezone is UTC. Changing only Flyway to `TIMESTAMP WITH TIME ZONE` would make the baseline diverge from the existing DB. |
