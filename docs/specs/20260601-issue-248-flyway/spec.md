...
- Existing dev/prod databases must be baselined manually before application migration is allowed.
- `baseline-on-migrate` stays disabled so an unprepared non-empty DB fails fast instead of silently creating a baseline.
...
1. Dev DB
   - Run `flyway baseline -baselineVersion=1` against the dev DB.
   - Run `flyway migrate` against the dev DB.
...
2. Prod DB
   ...
   - Run `flyway baseline -baselineVersion=1` against the prod DB.
   - Run `flyway migrate` against the prod DB.
   - Confirm `flyway_schema_history` contains baseline version `1` and successful version `2`.
