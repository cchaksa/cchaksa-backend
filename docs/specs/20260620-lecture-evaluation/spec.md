...
- Scope
  - In:
    - `semester_academic_records` 강의평가 필요/완료 상태 컬럼 추가
    - `lectureevaluations` 신규 패키지 추가
    - 강의평가 필요 여부와 성적 카드 조회 API 추가
...
- Rule 2:
  - 현재 수료 학기 과목 중 하나라도 `student_courses.grade`가 `IP`에서 완료 성적으로 바뀌면 `lecture_evaluation_required = true`로 표시한다.
- Rule 3:
  - 이미 `lecture_evaluation_completed = true`인 학기는 다시 평가 필요 상태로 되돌리지 않는다.
- Rule 4:
...
- Steps:
  ...
  3. 해당 `semester_academic_records` row의 `lecture_evaluation_required`를 true로 바꾼다.
  ...
  7. 백엔드가 평가 본문과 태그를 저장하고 `lecture_evaluation_completed`를 true로 바꾼다.
- Expected Result:
  - 평가 데이터가 저장되고 같은 학기에 다시 제출할 수 없다.

### Boundary Flow: 평가 필요 없음
- Condition:
  - target 학기의 `lecture_evaluation_required`가 false이거나 `lecture_evaluation_completed`가 true이다.
- Expected Behavior:
  - 조회 API는 `lectureEvaluationRequired = false`를 반환하고 `grades`는 빈 배열을 반환한다.
...
- Transaction Start Point:
  ...
  - 제출: `LectureEvaluationService#submit`
- Transaction End Point:
  - 포털 동기화: 변경된 수강 기록과 학기 평가 상태 flush
  - 제출: 평가 본문/태그 저장과 완료 상태 갱신
- Atomicity Scope:
  - 제출 API는 평가 저장과 `lecture_evaluation_completed = true` 갱신을 같은 트랜잭션에서 처리한다.
...
- Error Code: 신규 강의평가 대상 없음
  - Condition:
    - 제출 시 현재 학기가 `lecture_evaluation_required = true` and `lecture_evaluation_completed = false` 상태가 아니다.
  - Handling Layer:
