...
- Domain impact: `UserPortalConnectionRepository`의 기존 학생 재사용 경로에서 삭제된 엔티티 메서드 호출 제거.
...
- Consistency expectations: 기존 학사 데이터 삭제 후 학생 정보와 사용자 연결 저장이 같은 트랜잭션에서 완료된다.

## Testing Strategy
- Domain tests: `UserPortalConnectionRepositoryTests`에서 `resetAcademicData` 검증을 `resetBy(studentId)` 검증으로 변경.
