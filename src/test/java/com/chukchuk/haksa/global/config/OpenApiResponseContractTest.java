...
class OpenApiResponseContractTest {
  ...
  private static final List<OperationRef> PUBLIC_OPERATIONS =
      List.of(
  ...
  void openApiSecurityMatchesRuntimeAuthenticationRequirements() throws Exception {
    ...
    for (OperationRef operation : PUBLIC_OPERATIONS) {
      assertThat(
              operation(apiDocs, operation).path("security").isMissingNode()
  ...
  void callbackHealthAndSentryResponsesMatchRuntimeShape() throws Exception {
    ...
    assertJsonResponseRef(
        apiDocs, "/internal/scrape-results", "post", "200", "PortalLinkCallbackApiResponse");
    assertJsonResponseRef(
        apiDocs, "/internal/scrape-results", "post", "401", "ErrorResponseWrapper");

    assertTextResponse(apiDocs, "/health", "get", "200");

    JsonNode sentryResponses =
        operation(apiDocs, new OperationRef("/sentry-test", "get")).path("responses");
  ...
  @Test
  void generatedOpenApiDocumentsMissionLiberalAreaCodeForGraduationAndAcademicRecord()
      throws Exception {
    JsonNode apiDocs = apiDocs();
    ...
    JsonNode graduationCourse =
        arrayItemSchema(apiDocs, propertySchema(apiDocs, graduationProgress, "courses"));
    ...
    JsonNode studentCourse =
        arrayItemSchema(apiDocs, propertySchema(apiDocs, academicRecordCourses, "liberal"));
                        ...
                        methodEntry ->
                            methodEntry
                                ...
                                .forEachRemaining(
                                    responseEntry ->
                                        assertThat(
                                                responseEntry.getValue().path("content").has("*/*"))
                                            .as(
                                                "%s %s %s should not use wildcard media type",
                                                methodEntry.getKey(),
                                                pathEntry.getKey(),
  ...
  private void assertJsonResponseRef(
      JsonNode apiDocs, String path, String method, String responseCode, String schemaName) {
    JsonNode response =
  ...
  private void assertTextResponse(
      JsonNode apiDocs, String path, String method, String responseCode) {
    JsonNode response =
        apiDocs.path("paths").path(path).path(method).path("responses").path(responseCode);
  ...
  private JsonNode propertySchema(JsonNode apiDocs, JsonNode schema, String propertyName) {
    ...
    assertThat(property.isMissingNode())
        .as("schema property %s should exist", propertyName)
        .isFalse();
